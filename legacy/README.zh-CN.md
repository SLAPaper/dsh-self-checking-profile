...
> 这是 **legacy profile/fork 路线**。单包 plugin 路线位于
> [`../plugin/`](../plugin/README.md)；两条路线的对比见仓库根 README。
> 在仓库根目录请使用 `../install-legacy.sh` / `../install-legacy.ps1`；
> 在本目录中下面的 `install.sh` / `install.ps1` 仍可直接使用。

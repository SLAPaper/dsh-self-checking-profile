...
> This is the **legacy profile/fork route**. The single-package plugin route
> lives in [`../plugin/`](../plugin/README.md); see the repository root README
> for a comparison. From the repository root use `../install-legacy.sh` /
> `../install-legacy.ps1`; from this directory the `install.sh` / `install.ps1`
> scripts below work unchanged.
